...
/**
 * Lighthouse CI configuration for the Bitbucket Pipelines port.
 * Scoped to Platypus only (not all 8 GRA storefronts) to fit the Bitbucket
 * workspace's Free-tier build-minute budget (50 min/month). lighthouserc.js
 * (used by the GitHub Actions workflow) still audits all 8 storefronts.
 *
 * Run locally:  npx lhci autorun --config=lighthouserc.bitbucket.js
 * Collect only: npx lhci collect --config=lighthouserc.bitbucket.js
 * Assert only:  npx lhci assert --config=lighthouserc.bitbucket.js
 */
