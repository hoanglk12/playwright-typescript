...
**Default panel deliberately mixes providers across two data-residency zones** — an explicit user decision, not an oversight: `openai/gpt-5.4-mini` (OpenAI, US), `google/gemini-3.1-flash-lite` (Google, US), `anthropic/claude-haiku-4.5` (Anthropic, US), `x-ai/grok-4` (xAI, US), and `moonshot/kimi-k3` (Moonshot AI, China). `--panel=quality` and `--panel=smoke` are named overrides (see `scripts/council/panels.mjs`).

**Persona-per-panelist is the default prompt style** (`--neutral` switches to Karpathy's original neutral prompt for all models): `openai/gpt-5.4-mini` = Contrarian, `google/gemini-3.1-flash-lite` = First Principles Thinker, `anthropic/claude-haiku-4.5` = Expansionist, `x-ai/grok-4` = Outsider, `moonshot/kimi-k3` = Executor. Persona descriptions are copied verbatim from the Claude-subagent skill so both tools use the same thinking-style definitions.
