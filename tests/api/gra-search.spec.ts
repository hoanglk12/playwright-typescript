...
test.describe('GRA Search API @api @graphql @regression', () => {
  ...
  test('TC_01 - products - valid search term returns non-empty results with aggregations', async ({ createGraphQLClient, site }) => {
    ...
    await logger.step(`Step 1 - Execute ProductSearch query with valid search term "${site.catalogSearchTerm}"`, async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_QUERY, {
    ...
    await logger.step('Step 3 - Assert results non-empty', async () => {
  ...
  test('TC_02 - products - valid search term returns items with expected shape', async ({ createGraphQLClient, site }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearch query with valid search term', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_QUERY, {
        search: site.catalogSearchTerm,
        pageSize: GraSearchData.pagination.pageSize,
    ...
    let data: any;
    await logger.step('Step 3 - Assert at least one item is returned', async () => {
      data = await response.getData();
      expect(data.products.items.length).toBeGreaterThan(0);
    });

    await logger.step('Step 4 - Assert items have expected shape (sku, name fields)', async () => {
  ...
  test('TC_03 - products - nonsense search term returns empty results', async ({ createGraphQLClient }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearch query with nonsense term', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_QUERY, {
    ...
    await logger.step('Step 2 - Assert no server error (query succeeds)', async () => {
      await response.assertNoErrors();
      await response.assertHasData();
    });

    await logger.step('Step 3 - Assert empty results', async () => {
      const data = await response.getData();
  ...
  test('TC_04 - products - special characters in search term handled gracefully', async ({ createGraphQLClient }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearch query with special characters', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_QUERY, {
    ...
    await logger.step('Step 2 - Assert no server error (graceful handling)', async () => {
      await response.assertNoErrors();
      await response.assertHasData();
    });

    await logger.step('Step 3 - Assert result shape is valid', async () => {
      const data = await response.getData();
  ...
  test('TC_05 - products - empty string search handled gracefully', async ({ createGraphQLClient }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearch query with empty string', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_QUERY, {
    ...
    await logger.step('Step 2 - Handle both valid response and GraphQL error gracefully', async () => {
      gqlResponse = await response.getGraphQLResponse();
      hasErrors = (gqlResponse.errors?.length ?? 0) > 0;
    });

    await logger.step('Step 3 - Assert no server crash (HTTP-level)', async () => {
  ...
  test('TC_06 - productSearch - short autocomplete phrase returns suggestions or schema-not-supported error', async ({ createGraphQLClient }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearchSuggestions query with short phrase', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_SUGGESTIONS_QUERY, {
        phrase: GraSearchData.searchTerms.autocomplete,
        page_size: GraSearchData.pagination.pageSize,
      });
    });

    // isSchemaError must be readable at test scope (not inside the step callback below) so the
    // early `return` that follows aborts the whole test, not just the step.
    let isSchemaError = false;
    await logger.step('Step 2 - Handle: field supported (data returned) OR schema error (field not in this API version)', async () => {
      const gqlResponse = await response.getGraphQLResponse();
      const hasErrors = (gqlResponse.errors?.length ?? 0) > 0;
      isSchemaError = hasErrors && gqlResponse.errors!.some(
    ...
    await logger.step('Step 3 - Field available: assert suggestion items are defined', async () => {
      await response.assertNoErrors();
      await response.assertHasData();
      const data = await response.getData();
  ...
  test('TC_07 - productSearch - no-match phrase returns empty suggestions or schema-not-supported error', async ({ createGraphQLClient }) => {
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Execute ProductSearchSuggestions query with no-match phrase', async () => {
      const client = await createGraphQLClient();
      response = await client.queryWrapped(PRODUCT_SEARCH_SUGGESTIONS_QUERY, {
    ...
    await logger.step('Step 3 - Field available: assert response shape is valid (items may be empty)', async () => {
      await response.assertNoErrors();
      await response.assertHasData();
      const data = await response.getData();
      softExpect(data.productSearch).toBeDefined();
