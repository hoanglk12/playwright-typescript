...
test.describe("GRA GraphQL API - My Details apis", () => {
  ...
  test("GRA_UpdateCustomerAddressInAddressBook - should update customer address with valid token", async ({
    ...
    let variables!: { addressId: string; updated_address: unknown };
    await logger.step('Step 1 - Verify prerequisites', async () => {
      ...
      authClient = await createGraphQLClient({
        authType: AuthType.BEARER,
        token: customerToken,
      });
    ...
    await logger.step('Step 3 - Assert response', async () => {
  ...
  test("GRA_DeleteCustomerAddressFromAddressBook - should delete customer address with valid token", async ({
    ...
    let authClient!: GraphQLClient;
    let variables!: { addressId: string };
    await logger.step('Step 1 - Verify prerequisites', async () => {
    ...
    let data: any;
    await logger.step('Step 2 - Send DeleteCustomerAddress mutation', async () => {
  ...
  test("GRA_loyalty - retrieve loyalty newsletter subscription message with valid token", async ({
    ...
    let data: any;
    await logger.step('Step 1 - Send loyalty query with bearer token', async () => {
      const authClient = await createGraphQLClient({
        authType: AuthType.BEARER,
        token: customerToken,
      });
