...
test.describe('DummyJSON Users API @api @regression', () => {
  test('TC_01 - Should list users with pagination metadata', async ({ dummyjsonService, softAssert }) => {
    ...
    await logger.step('Step 1 - GET /users', async () => {
      response = await dummyjsonService.getAllUsers();
    });
  ...
  test('TC_02 - Should get a user by known ID', async ({ dummyjsonService }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - GET /users/{id}', async () => {
      response = await dummyjsonService.getUserById(DUMMYJSON_KNOWN_USER_ID);
    });
  ...
  test('TC_03 - Should return 404 for an unknown user ID', async ({ dummyjsonService }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - GET /users/{unknownId}', async () => {
      response = await dummyjsonService.getUserById(DUMMYJSON_UNKNOWN_ID);
    });
  ...
  test('TC_04 - Should search users by query string', async ({ dummyjsonService }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - GET /users/search?q=emily', async () => {
      response = await dummyjsonService.searchUsers('emily');
    });
  ...
  test('TC_05 - Should add a new user (simulated)', async ({ dummyjsonService, softAssert }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - POST /users/add', async () => {
      response = await dummyjsonService.addUser(newUser);
    });

    await logger.step('Step 2 - Assert simulated user echoes submitted data', async () => {
  ...
  test('TC_06 - Should update a user (simulated)', async ({ dummyjsonService }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - PUT /users/{id}', async () => {
      response = await dummyjsonService.updateUser(DUMMYJSON_KNOWN_USER_ID, update);
    });
  ...
  test('TC_07 - Should delete a user (simulated)', async ({ dummyjsonService, softAssert }) => {
    ...
    let response!: ApiResponseWrapper;
    await logger.step('Step 1 - DELETE /users/{id}', async () => {
      response = await dummyjsonService.deleteUser(DUMMYJSON_KNOWN_USER_ID);
    });
