...
test.describe('GRA Customer Profile @api @graphql @regression', () => {
  ...
  test('TC_01 - changeCustomerPassword - valid credentials should update password successfully', async ({
    ...
    await logger.step('Step 1 - Change password to temp password', async () => {
      ...
      changeData = await changeResponse.getData();
    });

    await logger.step('Step 2 - Assert password change returned customer object', async () => {
  ...
  test('TC_02 - changeCustomerPassword - wrong current password should return authentication error', async ({
    ...
    await logger.step('Step 1 - Attempt password change with incorrect current password', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(
        CHANGE_PASSWORD_MUTATION,
        graCustomerProfileData.invalidCurrentPassword
      );
    });

    await logger.step('Step 2 - Assert authentication error is returned', async () => {
  ...
  test('TC_03 - changeCustomerPassword - weak new password should return validation error', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Attempt password change with a weak new password', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(
        CHANGE_PASSWORD_MUTATION,
    ...
    await logger.step('Step 2 - Assert validation error is returned', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
  ...
  test('TC_04 - changeCustomerPassword - unauthenticated request should return UNAUTHORIZED error', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Call changeCustomerPassword without auth token', async () => {
      const publicClient = await createGraphQLClient();
      response = await publicClient.mutateWrapped(
        CHANGE_PASSWORD_MUTATION,
        graCustomerProfileData.validPasswordChange
      );
    });

    await logger.step('Step 2 - Assert authorization error is returned', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
  ...
  test('TC_05 - updateCustomerV2 - firstname+lastname update blocked by store password policy on staging', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Attempt to update firstname and lastname', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(UPDATE_NAME_MUTATION, {
        firstname: graCustomerProfileData.updatedFirstname,
        lastname: graCustomerProfileData.updatedLastname,
      });
    });

    await logger.step('Step 2 - Assert store returns password-required error (staging has Require Password for Account Changes enabled)', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
      const errorMsg = gqlResponse.errors?.length ? (gqlResponse.errors[0]?.message ?? '') : '';
  ...
  test('TC_06 - updateCustomerV2 - date_of_birth update blocked by store password policy on staging', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Attempt to update date_of_birth', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(UPDATE_DOB_MUTATION, {
        date_of_birth: graCustomerProfileData.updatedDateOfBirth,
      });
    });

    await logger.step('Step 2 - Assert store returns password-required error', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
      logger.verify('Password-required error returned', true, gqlResponse.errors!.length > 0);
      softExpect(gqlResponse.errors!.length).toBeGreaterThan(0);
  ...
  test('TC_07 - updateCustomerV2 - phone_number update blocked by store password policy on staging', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Attempt to update phone_number', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(UPDATE_PHONE_MUTATION, {
  ...
  test('TC_08 - updateCustomerV2 - email change without password should return error', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Send updateCustomerV2 with new email but no password', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(UPDATE_EMAIL_MUTATION, {
        email: graCustomerProfileData.emailChangeWithoutPassword.input.email,
      });
    });

    await logger.step('Step 2 - Assert error is returned for email change without password', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
  ...
  test('TC_09 - updateCustomerV2 - duplicate email should return already exists error', async ({
    ...
    let response!: GraphQLResponseWrapper;
    await logger.step('Step 1 - Send updateCustomerV2 attempting to claim an already-registered email', async () => {
      const authClient = await createGraphQLClient({ authType: AuthType.BEARER, token: customerToken });
      response = await authClient.mutateWrapped(UPDATE_EMAIL_MUTATION, {
    ...
    await logger.step('Step 2 - Assert error is returned (duplicate or password-required)', async () => {
      await response.assertHasErrors();
      const gqlResponse = await response.getGraphQLResponse();
