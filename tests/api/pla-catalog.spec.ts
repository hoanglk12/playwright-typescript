...
test.describe('PLA Catalog & Products API @api @graphql @regression', () => {
  ...
  test('TC_01 - products PLP - category filter returns paginated product list', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Guard: category filter must have been discovered');
    ...
    logger.step('Step 2 - Query products with discovered category filter');
    const client = await createGraphQLClient();
    ...
    logger.step('Step 3 - Assert no critical errors and paginated product list returned');
  ...
  test('TC_02 - products PLP - brand filter returns filtered product list', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Check if brand_id was discovered');
    if (!discoveredBrandId) {
      logger.action('Skip', 'TC_02 — apparel21_brand_id aggregation not returned by staging; brand filter test not applicable');
      return;
    }

    logger.step('Step 2 - Query products with apparel21_brand_id filter');
    const client = await createGraphQLClient();
    const response = await client.queryWrapped(PLP_QUERY, {
    ...
    logger.step('Step 3 - Assert no critical errors and filtered list returned');
  ...
  test('TC_06 - products PLP - no-match filter returns empty result', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 2 - Assert either GraphQL error or empty result (both are valid)');
  ...
  test('TC_07 - products PDP - fetch product by url_key returns product details', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Guard: product url_key must have been discovered');
    ...
    logger.step('Step 4 - Verify product detail fields');
  ...
  test('TC_08 - products PDP - price_range structure is valid', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 3 - Verify price_range.minimum_price.final_price structure');
  ...
  test('TC_09 - products PDP - configurable product variants have stock_status', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 3 - Assert variants have stock_status (configurable product)');
  ...
  test('TC_11 - categories - root category tree returns children with names and url_keys', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Query categories without filter to get root tree');
    const client = await createGraphQLClient();
    const response = await client.queryWrapped(CATEGORIES_QUERY, {});

    logger.step('Step 2 - Assert no errors and category tree returned');
    await response.assertNoErrors();
    await response.assertHasData();
    const data = await response.getData();
    ...
    logger.step('Step 3 - Verify root category children have names and url_keys');
  ...
  test('TC_12 - categories - fetch category by url_key returns category details', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Guard: category url_key must have been discovered');
    expect(discoveredCategoryUrlKey, 'discoveredCategoryUrlKey must be set by beforeAll').toBeTruthy();

    logger.step('Step 2 - Query category by url_key');
    const client = await createGraphQLClient();
    const response = await client.queryWrapped(CATEGORIES_QUERY, {
      filters: { url_key: { eq: discoveredCategoryUrlKey } },
    });

    logger.step('Step 3 - Assert category details returned');
    await response.assertNoErrors();
    await response.assertHasData();
    const data = await response.getData();
    logger.verify('categories data present', true, data.categories != null);
  ...
  test('TC_14 - storeConfig - store_code and id return valid values', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 1 - Query storeConfig');
    const client = await createGraphQLClient();
    const response = await client.queryWrapped(STORE_CONFIG_QUERY);
    ...
    logger.step('Step 3 - Verify store_code and id fields');
  ...
  test('TC_15 - storeConfig - ewave_dynamicpromoblocks enable flags are Boolean', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 3 - Verify all four promo block flags are Boolean');
  ...
  test('TC_17 - urlResolver - product URL resolves to type PRODUCT with id', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 2 - Resolve product URL');
    const client = await createGraphQLClient();
    ...
    logger.step('Step 3 - Assert PRODUCT type returned (try with and without .html suffix)');
  ...
  test('TC_18 - urlResolver - category URL resolves to type CATEGORY with id', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 2 - Resolve category URL');
    const client = await createGraphQLClient();
    ...
    logger.step('Step 3 - Assert CATEGORY type returned');
  ...
  test('TC_19 - urlResolver - CMS page URL resolves to type CMS_PAGE with id', async ({ createGraphQLClient }) => {
    ...
    logger.step('Step 2 - Assert CMS_PAGE type returned (or note if page not present on staging)');
