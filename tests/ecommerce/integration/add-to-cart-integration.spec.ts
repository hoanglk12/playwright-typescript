...
test.describe('Ecommerce Add to Cart Integration @ecommerce @integration @regression', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Add to Cart mutation propagates to UI and analytics`, async ({
      ...
      logger.step('Step 1 - Intercept add-to-cart GraphQL mutation before navigation (pass-through only)');
      const capture = await createAddToCartCapture(page);

      logger.step('Step 2 - Navigate to PLP');
      await navigateToPlp(ecommerceNavPage, ecommercePLPPage, site, navLabel);

      logger.step('Step 3 - Scan PLP for a product with available sizes (precondition)');
      ...
      logger.step('Step 4 - Capture product name and price BEFORE size selection');
      const productName = await ecommercePDPPage.getProductName();
      const productPrice = await ecommercePDPPage.getPrice();

      logger.step('Step 5 - Capture initial mini cart count before ATC');
      ...
      logger.step('Step 6 - Select a size, then Add to Cart immediately (try up to 3 sizes)');
      ...
      logger.step('Step 7 - Poll for mini cart count to increment after ATC');
      const newCartCount = await ecommercePDPPage.waitForMiniCartCountIncrement(initialCartCount);

      logger.step('Step 8 - Dismiss Vans Bloomreach popup if present, then open the mini cart overlay');
      await dismissVansPostAtcPopup(ecommercePDPPage, site);
      await ensureCartOverlayOpen(ecommerceCartOverlayPage);

      logger.step('Step 9 - Boundary A: Assert intercepted GraphQL mutation cart has items');
      ...
      logger.step('Step 10 - Boundary B: Assert cart badge equals initial count + 1 (delta assertion)');
      softAssert.toBe(
        newCartCount,
        initialCartCount + 1,
      ...
      logger.step('Step 11 - Boundary B: Assert mini-cart overlay total matches PDP price (±0.01)');
      ...
      logger.step('Step 12 - Boundary C: Assert add-to-cart event fired in adobeDataLayer/dataLayer');
      const atcEvent = await getAddToCartDataLayerEvent(page);
      softAssert.toBeTruthy(
        !!atcEvent,
        `${site.name}: add_to_cart event should be present in adobeDataLayer or dataLayer`,
      );

      logger.step('Step 13 - Boundary C: Assert event product name matches the PDP product name captured at Step 4');
