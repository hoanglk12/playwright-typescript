...
test.describe('Ecommerce Navigation Smoke @ecommerce @smoke @navigation', () => {
  ...
  for (const [index, site] of navStorefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} all top-nav links render and are clickable`, async ({ ecommerceNavPage, softAssert }) => {
      ...
      await logger.step('Step 2 - Wait for nav hydration', async () => {
        await ecommerceNavPage.waitForNavHydration();
      });

      await logger.step('Step 3 - Assert all top-nav links are visible and have valid hrefs', async () => {
  ...
  for (const [index, site] of womensSites.entries()) {
    ...
    test(`${tcId} - ${site.name} ${site.womensNavLabel} link navigates to womens PLP`, async ({ ecommerceNavPage }) => {
      ...
      await logger.step('Step 3 - Click women\'s nav link', async () => {
        await ecommerceNavPage.clickNavLink(site.womensNavLabel!);
      });

      await logger.step('Step 4 - Assert URL navigates to women\'s PLP', async () => {
  ...
  for (const [index, site] of mensSites.entries()) {
    ...
    test(`${tcId} - ${site.name} ${site.mensNavLabel} link navigates to mens PLP`, async ({ ecommerceNavPage }) => {
      ...
      await logger.step('Step 3 - Click men\'s nav link', async () => {
        await ecommerceNavPage.clickNavLink(site.mensNavLabel!);
      });

      await logger.step('Step 4 - Assert URL navigates to men\'s PLP', async () => {
  ...
  for (const [index, site] of kidsSites.entries()) {
    ...
    test(`${tcId} - ${site.name} ${site.kidsNavLabel} link navigates to kids PLP`, async ({ ecommerceNavPage }) => {
      ...
      await logger.step('Step 3 - Click kids nav link', async () => {
        await ecommerceNavPage.clickNavLink(site.kidsNavLabel!);
      });

      await logger.step('Step 4 - Assert URL navigates to kids PLP', async () => {
  ...
  for (const [index, site] of saleSites.entries()) {
    ...
    test(`${tcId} - ${site.name} ${site.saleNavLabel} link navigates to sale PLP`, async ({ ecommerceNavPage }) => {
      ...
      await logger.step('Step 3 - Click sale nav link', async () => {
        await ecommerceNavPage.clickNavLink(site.saleNavLabel!);
      });

      await logger.step('Step 4 - Assert URL navigates to sale PLP', async () => {
  ...
  for (const site of storefronts) {
    ...
    test(`E2E-NAV-009 - ${site.name} logo click returns to homepage`, async ({ ecommerceNavPage }) => {
      ...
      await logger.step(`Step 3 - Click "${interiorLabel}" nav link to leave homepage`, async () => {
        await ecommerceNavPage.clickNavLink(interiorLabel);
      });

      await logger.step('Step 4 - Confirm we navigated away from root', async () => {
      ...
      await logger.step('Step 5 - Click the site logo', async () => {
        await ecommerceNavPage.clickLogo();
      });

      await logger.step('Step 6 - Assert URL returned to site root', async () => {
