...
import type { FreshAccountCredentials } from '@data/ecommerce/test-accounts';
...
test.describe('Ecommerce Auth Smoke @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Login modal opens via account icon in header`, async ({
      ...
      await logger.step('Step 4 - Assert login modal is visible', async () => {
      ...
      await logger.step('Step 5 - Assert login form inputs and button are visible', async () => {
      ...
      await logger.step('Step 6 - Assert modal title is non-empty', async () => {
...
test.describe('Ecommerce Auth - Login @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Successful login with valid credentials`, async ({
      ...
      await logger.step('Step 3 - Navigate to storefront homepage', async () => {
        await ecommerceAccountModalPage.navigate(site.url);
      });

      await logger.step('Step 4 - Click account icon to open login modal', async () => {
        await ecommerceAccountModalPage.openModal();
      });

      await logger.step('Step 5 - Wait for modal to appear', async () => {
        await ecommerceAccountModalPage.waitForModalVisible();
      });

      await logger.step('Step 6 - Assert login modal is visible before submitting credentials', async () => {
      ...
      await logger.step('Step 7 - Fill fresh credentials and click Login', async () => {
        await ecommerceAccountModalPage.login(creds.email, creds.password);
      });

      await logger.step('Step 8 - Wait for login to complete (best-effort)', async () => {
        await ecommerceAccountModalPage.waitForLoginComplete();
      });

      await logger.step('Step 9 - Assert login success signals', async () => {
...
test.describe('Ecommerce Auth - Invalid Login @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Failed login with invalid password shows error`, async ({
      ...
      await logger.step('Step 6 - Fill invalid credentials and click Login', async () => {
...
test.describe('Ecommerce Auth - Non-existent Email Login @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Failed login with non-existent email shows error`, async ({
      ...
      await logger.step('Step 6 - Fill non-existent email credentials and click Login', async () => {
      ...
      await logger.step('Step 8 - Assert error appeared only after failed login attempt', async () => {
        const errorAfterSubmit = await ecommerceAccountModalPage.getLoginErrorMessage();
        softAssert.toBeTruthy(
          errorBeforeSubmit.length === 0 && errorAfterSubmit.length > 0,
...
test.describe('Ecommerce Auth - Brand Title @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Login modal title matches brand`, async ({
      ...
      await logger.step('Step 4 - Assert login modal is visible (precondition guard)', async () => {
        const modalVisible = await ecommerceAccountModalPage.isModalVisible();
        logger.verify('Login modal visible', true, modalVisible);
        expect(modalVisible).toBe(true);
      });

      await logger.step('Step 5 - Assert login panel contains the brand name (retrying until CMS block renders)', async () => {
...
test.describe('Ecommerce Auth - Logout @ecommerce @smoke @auth', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Logout clears session and redirects`, async ({
      ...
      await logger.step('Step 8 - Wait for login to complete', async () => {
        await ecommerceAccountModalPage.waitForLoginComplete();
      });

      await logger.step('Step 9 - Assert login success signal (precondition for logout test)', async () => {
      ...
      await logger.step('Step 10 - Open account panel while authenticated', async () => {
        await ecommerceAccountModalPage.openModalWhenLoggedIn();
      });

      await logger.step('Step 11 - Click logout button', async () => {
        await ecommerceAccountModalPage.logout();
      });

      await logger.step('Step 12 - Wait for login form — panel transitions in place on most storefronts', async () => {
        await ecommerceAccountModalPage.waitForLoginFormVisible();
      });

      await logger.step('Step 13 - Re-open account panel only if it closed post-logout', async () => {
      ...
      await logger.step('Step 14 - Assert login form is visible (positive logout confirmation)', async () => {
      ...
      await logger.step('Step 15 - Soft-assert individual login form inputs are visible', async () => {
