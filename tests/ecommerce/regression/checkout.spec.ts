...
test.describe('Ecommerce Checkout Regression @regression @ecommerce', () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Checkout page loads after items added to cart`, async ({
      ...
      await logger.step('Step 15 - Assert checkout page has loaded', async () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Guest checkout email entry step is presented`, async ({
      ...
      await logger.step('Step 15 - Assert guest checkout email field is presented', async () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Shipping address form validates required fields`, async ({
      ...
      await logger.step('Step 17 - Submit the blank shipping form to trigger required-field validation', async () => {
        await ecommerceCheckoutPage.submitCurrentStep();
      });

      await logger.step('Step 18 - Assert required-field validation is shown on the shipping form', async () => {
      ...
      await logger.step('Step 19 - Capture validation messages for logging (observational, independent check)', async () => {
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Shipping method selection updates order total`, async ({
      ...
      let methodCount = 0;
      await logger.step('Step 19 - Assert at least 1 selectable shipping method is available (precondition)', async () => {
        methodCount = await ecommerceCheckoutPage.getSelectableShippingMethodCount();
      ...
      await logger.step('Step 20 - Read the current order summary totals and detect whether a method already arrived pre-selected', async () => {
        ...
        currentTotals = await ecommerceCheckoutPage.getOrderSummaryTotals();
        preSelected = await ecommerceCheckoutPage.isAnyShippingMethodSelected();
      ...
      if (!preSelected) {
        let totalsAfterFirst: OrderSummaryTotals = { subtotal: null, delivery: null, total: null, discount: null };
        let deliveryIncreasedFromBaseline = false;
        await logger.step('Step 21 - No method pre-selected: select the first available method and capture the settled delivery cost', async () => {
          await ecommerceCheckoutPage.selectNthEnabledShippingMethod(0);
          totalsAfterFirst = await ecommerceCheckoutPage.waitForOrderSummaryTotalChange(
        ...
        if (deliveryIncreasedFromBaseline) {
          ...
          await logger.step('Step 22 - Assert the delivery cost increased from the pre-selection $0 baseline (final outcome check)', async () => {
            softAssert.toBeTruthy(
              deliveryIncreasedFromBaseline,
          ...
          await logger.step('Step 22 - First method is itself free: select an alternate method and compare delivery costs against each other (final outcome check)', async () => {
          ...
          await logger.step('Step 22 - Only method is genuinely free: confirm the selection registered and assert internal consistency of the settled totals', async () => {
        ...
        let checkedIndex = -1;
        await logger.step('Step 21 - A method is pre-selected and more than 1 is available: select an alternate method and wait for the delivery cost to change', async () => {
          checkedIndex = await ecommerceCheckoutPage.getCheckedShippingMethodIndex();
        });
        if (checkedIndex === -1) {
          ...
          await logger.step('Step 22 - Checked method index could not be determined: assert internal consistency of the settled totals instead of guessing an alternate', async () => {
          ...
          await logger.step('Step 22 - Assert the delivery cost changed after switching to the alternate method (final outcome check)', async () => {
            const deliveryChanged =
              currentTotals.delivery !== null &&
        ...
        await logger.step('Step 21 - Exactly 1 method, pre-selected: assert internal consistency of the settled totals (already proves the requirement)', async () => {
          assertSingleMethodConsistency(currentTotals);
        });
      }
    });
  ...
  for (const [index, site] of storefronts.entries()) {
    ...
    test(`${tcId} - ${site.name} Order review shows correct items, quantities, total`, async ({
      ...
      let reviewedItem!: OrderReviewLineItem;
      await logger.step('Step 19 - Read the order-review line items (precondition for per-item checks)', async () => {
        ...
        reviewedItem = lineItems[0];
      });

      await logger.step('Step 20 - Assert the line item name matches the product added at ATC time (independent check)', async () => {
      ...
      await logger.step('Step 21 - Assert the line item quantity matches the quantity actually added (independent check)', async () => {
        softAssert.toBe(
      ...
      await logger.step('Step 22 - Assert the line item price matches the price captured at ATC time (independent check)', async () => {
      ...
      await logger.step('Step 23 - Assert the order summary total is internally consistent (final outcome check)', async () => {
