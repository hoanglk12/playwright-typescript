...
**Addendum, 2026-08-02 (third): `kimi-k3` empty-content bug diagnosed as the same class as `glm-5.2`; static pricing added.** A later real run (`login-page.ts`) hit `kimi-k3` failing with empty completion content at the (post-temperature-fix) default `max_tokens: 1200`. Live investigation confirmed it is the identical reasoning-budget-exhaustion pattern documented above for `glm-5.2`, not a new defect: replaying the exact failing prompt reproduced `finish_reason: "length"` with 1197 of 1200 completion tokens spent on `reasoning_content` and 0 on `content`. Fix applied: `tiers.mjs`'s new `MODEL_MAX_TOKENS_OVERRIDE` raises kimi-k3's default budget to 8000 (an explicit `--max-tokens` still overrides this per-model default for every model in the panel); `client.mjs`'s `chat()` now detects the `finish_reason==="length"` + non-empty `reasoning_content` signature and marks that specific `EMPTY_CONTENT` as non-retryable, since retrying at the same budget would deterministically re-fail and re-bill the prompt for nothing (the triggering run burned 3 such retries, ≈$0.59, for zero output, now knowable because of the pricing work below). **Not yet empirically confirmed end-to-end**: verification at `max_tokens: 8000` against the real 15.6KB prompt hit transient ShopAIKey capacity errors (HTTP 529 `overloaded_error`, HTTP 503 "no available channel") twice, unrelated to this bug, before a successful non-empty completion could be captured. Re-verify with a real run.

Separately, `GET /v1/models` on ShopAIKey was confirmed live to exist and return 200 with a 349-model catalog including both `kimi-k3` and `gpt-5.6-sol` — contradicting this report's and `tiers.mjs`'s earlier "may not expose a catalog endpoint at all" / "unverified" language, which is now stale (§1, §3.3.1, §8.6, §10, §12 below still describe the pre-verification state and are left as the historical record of that uncertainty, not corrected in place). That catalog response carries no `pricing` field, so it doesn't supply cost data — real pricing (`kimi-k3`: $20/$100 input/completion per 1M tokens; `gpt-5.6-sol`: $5/$40) was instead scraped from the embedded data payload of the public `https://shopaikey.com/models` pricing page and hardcoded into `tiers.mjs`'s `PRICING_PER_MILLION_TOKENS_USD`, dated 2026-08-02. The consent manifest's cost estimate and the end-of-run actual-cost line now use this static pricing instead of always printing "UNKNOWN" — see `CLAUDE.md` "LLM Council" section for the current, authoritative behavior. `--max-cost` remains deliberately unenforced (informational only); this was already the documented design intent (the `--yes` consent step is the real backstop), not a consequence of unknown pricing, so nothing here changes that decision.
